...
assertSolanaIrysWriteEnabled('Solana/Irys script write surface: boa-pipeline/burn-dupes3.mjs');
...
for (const mint of dupes) {
  try {
    ...
    if (!ataAcct.exists) {
      console.log("⏭️ Skipping", mint, "— deployer has no ATA");
      continue;
    }
    ...
    console.error("❌ Failed", mint, e.message?.slice(0,300) || e);
  }
}
