...
Introduce a future server-side allowlisted setting named `PAID_TRUST_X402_MODE` with values:
...
The setting must be read only by the paid trust-score route adapter, default to `enabled` until an independently reviewed release changes it, and emit value-free state/metrics. Activation requires exact-head review, rollback instructions, proof that unpaid trust/SATP reads remain reachable, and canonical live readback. Wave 0 adds no setting to production and disables nothing.
