...
- On-chain fee collection inside release/partial_release routes the platform percentage to the treasury (FriU1FEp…) — shipped as an explicit fail-closed capability boundary: live release routes return `ESCROW_V3_FEE_ROUTING_UNSUPPORTED` without producing a transaction while the certified deployed interface lacks the writable treasury account required for auditable fee routing. [#011685d4] [shipped]
