...
- Record the deployed mainnet escrow_v3 program as canonical runtime truth: program `HXCUWKR2NvRcZ7rNAJHwPcH6QAAWaLR4bRFbfyuDND6C`, ProgramData `Fg1DJyKX9CngiMihZxJY2zjaQ8T1PK5QuiVhNvJmeTqk`, extracted ELF `sha256=b70a7a7ea55f43da7bd3fc4f666e1374436bb9c8aeaa83cb2f0a2a970b603094` (290680 bytes). The uploaded IDL is interface truth but is stale by `ArithmeticOverflow`; `clawd-brainchain@27f38ff` is a hash-unverified provenance candidate, and the current AgentFolio source is an undeployed fork. Read-only archaeology in `docs/operational/AGENTFOLIO-ESCROW-DEPLOYED-SOURCE-ARCHAEOLOGY-20260808.md` exhausted known source/toolchain candidates without reproducing the deployed hash, so source == deployed == IDL certification is replaced by this deployed-as-canonical documentation; no redeploy is authorized. [#49e40f78] [#3258f2a8] [#ef7e4581] [shipped]
...
- USDC escrow support remains source-only: the repo source contains the SPL vault PDA / ATA / `transfer_checked` path and five USDC entrypoints (`create_usdc_escrow`, `release_usdc`, `partial_release_usdc`, `cancel_usdc`, `resolve_dispute_usdc`), but canonical mainnet program `HXCUWKR2NvRcZ7rNAJHwPcH6QAAWaLR4bRFbfyuDND6C` exposes no token surface. Its decoded IDL account `D2TVCWarEDQ3w3YFMpackzymm9MGQKeWd1p1pCeZmBcn` contains only 9 instructions: `cancel`, `close_escrow`, `create_escrow`, `extend_deadline`, `partial_release`, `raise_dispute`, `release`, `resolve_dispute`, `submit_work`. [#806be0c8] [pending]
