...
Deflation note for [#49e40f78]: the canonical mainnet program was upgraded on 2026-08-19, so the previously recorded `b70a7a7e...` deployed hash and its shipped marker are no longer runtime truth. The replacement binary is recorded below from read-only mainnet RPC; no chain write was performed for this reconciliation. [#580d4a47] remains shipped because its devnet e2e evidence is separate from canonical-mainnet escrow provenance and was against B1Se8SPx..., not canonical mainnet HXCUWKR2....
...
- Record the current deployed mainnet escrow_v3 runtime identity as canonical runtime truth: program `HXCUWKR2NvRcZ7rNAJHwPcH6QAAWaLR4bRFbfyuDND6C`, ProgramData `Fg1DJyKX9CngiMihZxJY2zjaQ8T1PK5QuiVhNvJmeTqk`. The 2026-08-24T15:28:18Z upgrade transaction `21jwie1FpQGvjV5yFQ6ofgcKPzp3hrM2DKtLGeyQ4XVr2DQg5LYg7fqira9XSsUTTbfJBM9V8yY8Pe1fchDimkVx` installed an allocated ProgramData payload of 346856 bytes with `sha256=4f21da13659cbe99a606b408a5f1d3523c0e41de20538028939bbb1b54c3cc0d`; stripping the 45-byte upgradeable-loader header and 15 trailing zero bytes yields a 346841-byte deployed ELF with `sha256=2f3bb05486f39d3f61a454905048b6e5732b798643405836b62a6d9795a20a6d`. A fresh read-only mainnet RPC readback derives ProgramData from bytes 4..36 of the executable Program account before hashing the ProgramData bytes after the 45-byte header; upgrade authority remains `Bq1niVKyTECn4HDxAJWiHZvRMCZndZtC113yj3Rkbroc`. This shipped marker certifies current deployed-runtime identity only. [#49e40f78] [#3258f2a8] [#ef7e4581] [shipped]
- Re-certify source, build-input, and published-IDL provenance against the current allocated `4f21da13...` / trimmed `2f3bb054...` runtime. PR #270's negative binding and `88058f43...` receipt are historical evidence for the superseded 2026-08-19 runtime, not the current deployment. Authoritative refresh PR #273 records the current SATP source candidate and build packet, the allocated-runtime match, and the 14-instruction source/published IDL match; the packet is merged and independently re-read from `main`. No redeploy or write action is authorized by this roadmap correction. [#49e40f78] [#3258f2a8] [#ef7e4581] [shipped]
...
- On-chain fee collection inside release/partial_release routes the platform percentage to the treasury (FriU1FEp…) — shipped by PR #284 at merge commit `4db50f4c6142c412c996d969a77f9085d1f9b0ef`, with deterministic executed transfer evidence for both routes and certified source/deployed/IDL alignment closed by HQ task `AGENTFOLIO-011685D4-MERGED-PROOF-CLOSE-20260829`; prior gate context remains in docs/operational/ESCROW-V3-SOURCE-DEPLOYED-IDL-READBACK-49e40f78.md. [#011685d4] [shipped]
- Fail closed on unsupported live fee routing: while the certified deployed interface lacks the writable treasury account required for auditable routing, release and partial_release return `ESCROW_V3_FEE_ROUTING_UNSUPPORTED` without producing a transaction. Merged PR #280 supplies the implementation and regression evidence; this boundary does not claim that treasury routing itself is built. [shipped]
- USDC escrow support remains pending: the source and current published IDL expose the SPL vault PDA / ATA / `transfer_checked` path and five USDC entrypoints (`create_usdc_escrow`, `release_usdc`, `partial_release_usdc`, `cancel_usdc`, `resolve_dispute_usdc`). Fresh read-only mainnet RPC decoding of current IDL account `4zNAR5DGuWuUnEbwGb7FzEVUUCx2xKca2bmHCeVpjQCJ` yields 14 instructions and `sha256=e8c142f27e225d8edc2f8f41e6fb698ebbb73f69d2fc078d5bf963234ebc8fa9`, including all five USDC entrypoints; the nine-instruction `D2TVCWar...` IDL is historical to the superseded runtime. Keep [#806be0c8] pending until the current source/runtime/IDL packet is merged and the USDC production flow has cross-host-auditable end-to-end evidence. [#806be0c8] [pending]
