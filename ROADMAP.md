...
- Make the AgentFolio production escrow runtime mainnet-correct before unpause: `GET /api/v3/escrow/health` must report top-level `network=mainnet` and `escrowProvenance.runtimeProgramId=HXCUWKR2NvRcZ7rNAJHwPcH6QAAWaLR4bRFbfyuDND6C`, and live-write construction must fail closed unless both predicates hold. [pending]
- Consumer mainnet escrow unpause and release authorization: redeployment no longer remains as a required step. The fleet-owned mainnet runtime/configuration prerequisite is tracked separately immediately above; this blocked item waits only for a separate explicit Owner authorization to enable live-funds writes. Current production readback remains `enabled=false`, `ownerAuthorized=false`, and `liveEscrowWritesAllowed=false`. Do not mark [#ed3999ac] shipped from deployed program capability alone. [#ed3999ac] [blocked] · owner-gated
