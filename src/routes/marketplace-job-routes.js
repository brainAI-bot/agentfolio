...
function stageFunding(db, options) {
  ...
  return db.transaction(() => {
    ...
    const escrowId = String(options.body.escrowReference || options.body.escrowId || '').trim();
    if (!/^[A-Za-z0-9:_-]{8,200}$/.test(escrowId)) throw new MarketplaceJobError(400, 'INVALID_ESCROW_REFERENCE', 'A staged escrowReference is required');
...
function verifyFunding(db, options) {
  ...
  return db.transaction(() => {
    ...
    if (!escrow || escrowReference !== escrow.id) throw new MarketplaceJobError(409, 'ESCROW_REFERENCE_MISMATCH', 'Verification reference does not match staged funding');
    const amountMinor = parseDecimalToMinorUnits(options.body.amount, job.budget_currency);
    const expectedMinor = exactMinorUnits(job, 'budget_amount', 'budget_amount_minor');
    if (amountMinor !== expectedMinor || String(escrow.amount_minor || '') !== expectedMinor) {
    ...
    const source = String(options.body.verificationSource || '').trim();
    if (source !== 'staged_runtime_readback') throw new MarketplaceJobError(400, 'INVALID_VERIFICATION_SOURCE', 'verificationSource must be staged_runtime_readback');
    const now = options.now || new Date().toISOString();
    db.prepare("UPDATE escrows SET status = 'funded', deposit_confirmed_at = ?, updated_at = ? WHERE id = ?").run(now, now, escrow.id);
    ...
    db.prepare(`INSERT INTO marketplace_funding_effects
      ...
      .run(id, job.id, escrow.id, amountMinor, job.budget_currency, source, gate.status, key, now);
    return fundingResponse(existingFundingEvent(db, job.id, key));
  })();
}
