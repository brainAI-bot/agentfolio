...
function registerSATPWriteRoutes(app) {
  ...
  app.post('/api/satp/register', async (req, res) => {
    try {
      ...
      const signer = satpWrite.loadKeypair(getRequiredSatpPlatformKeypairPath('SATP identity registration signer'));
  ...
  app.post('/api/satp/reputation/submit', async (req, res) => {
    try {
      ...
      const caller = satpWrite.loadKeypair(getRequiredSatpPlatformKeypairPath('SATP reputation recompute signer'));
