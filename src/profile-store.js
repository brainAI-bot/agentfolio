...
try {
  ...
  satpV3 = { client: createSATPClient({ rpcUrl: process.env.SOLANA_RPC_URL || 'https://mainnet.helius-rpc.com/?api-key=91c63e44-1c7a-4b98-830b-6135632565fb' }), agentIdHash };
    ...
    (async () => {
      try {
        ...
        if (newLevel > genesis.verificationLevel) {
          ...
          // Also bump reputation: +100k per verification level
          const newRep = Math.min(1000000, 500000 + (verifCount * 50000));
          if (newRep > genesis.reputationScore) {
            const repTx = await satpV3.client.buildUpdateReputation(signer.publicKey, profileId, newRep);
            repTx.transaction.sign(signer);
            const repSig = await satpV3.client.connection.sendRawTransaction(repTx.transaction.serialize());
            console.log(`[SATP V3] Reputation updated for ${profileId}: ${genesis.reputationScore} → ${newRep}, tx=${repSig}`);
          }
        }
      } catch (err) {
        console.error(`[SATP V3] Verification update failed for ${profileId}:`, err.message);
      }
    })();
  }
