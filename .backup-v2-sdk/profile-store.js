...
// SATP V3 SDK — Migrated to @brainai/satp-v3
let satpV3;
try {
  const { SatpV3Client, SatpV3Builders, agentIdHash, deriveGenesisPda, deserializeGenesis, PROGRAM_IDS } = require('@brainai/satp-v3');
  const { Connection, Keypair } = require('@solana/web3.js');
  const rpcUrl = process.env.SOLANA_RPC_URL || 'https://mainnet.helius-rpc.com/?api-key=HELIUS_API_KEY_REDACTED';
  const v3Client = new SatpV3Client(rpcUrl);
  const v3Connection = new Connection(rpcUrl, 'confirmed');
  
  // Compatibility wrapper — adapts new SDK to old satpV3.client interface
  satpV3 = {
    agentIdHash,
    client: {
      connection: v3Connection,
      async getGenesisRecord(agentId) {
        const result = await v3Client.getGenesis(agentId);
        if (!result || !result.data) return null;
        const record = deserializeGenesis(Buffer.from(result.data));
        return { ...record, pda: result.pda };
      },
      async buildCreateGenesisRecord(authority, agentId, params) {
        const ix = SatpV3Builders.createIdentity({
          authority: authority.toBase58 ? authority.toBase58() : authority,
      ...
      async buildUpdateVerification(authority, agentId, level) {
        const ix = SatpV3Builders.updateIdentity({
          authority: authority.toBase58 ? authority.toBase58() : authority,
          agentId,
      ...
      async buildUpdateReputation(authority, agentId, score) {
        const ix = SatpV3Builders.updateIdentity({
          authority: authority.toBase58 ? authority.toBase58() : authority,
          agentId,
  ...
  console.log('[SATP V3] @brainai/satp-v3 SDK loaded successfully');
} catch (e) {
