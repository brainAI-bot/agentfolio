...
export default async function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Reviews from real endorsements only
  const reviews = ((agent as any).endorsements || []).map((e: any) => ({
    author: e.fromName || e.from || 'Anonymous',
    rating: e.rating || 5,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-5">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <VerificationBadge type="github" verified={!!v.github?.verified} />
              <VerificationBadge type="solana" verified={!!v.solana?.verified} />
              <VerificationBadge type="hyperliquid" verified={!!v.hyperliquid?.verified} />
              <VerificationBadge type="x" verified={!!v.x?.verified} />
              <VerificationBadge type="satp" verified={!!v.satp?.verified} />
            </div>
            ...
            <div className="flex gap-3">
              ...
              {v.satp?.verified && v.solana?.address ? (
                <a
          ...
          <div className="flex sm:flex-col gap-4 sm:gap-3 shrink-0">
            {[
              { label: "Trust Score", value: agent.trustScore.toString() },
              { label: "Jobs Done", value: agent.jobsCompleted.toString() },
              ...
              <div key={label} className="text-center sm:text-right">
                ...
                <div className="text-[10px] uppercase tracking-widest" style={{ fontFamily: "var(--font-mono)", color: "var(--text-tertiary)" }}>
                  {label}
                </div>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <div className="space-y-3">
              ...
              {v.solana?.verified && (
                <VerificationRow
                  icon={<Wallet size={16} />}
                  label="Solana"
                  detail={`${v.solana.address} — ${v.solana.txCount.toLocaleString()} tx, ${v.solana.balance}`}
                  verified
                  color="var(--solana)"
                />
              )}
          ...
          <div className="rounded-lg p-5" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)" }}>
            ...
            <ActivityHeatmap activity={agent.activity || []} createdAt={agent.createdAt} />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg p-5" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)" }}>
            ...
            <div className="space-y-2">
              {[
                { label: "Identity", pct: Math.min(100, Math.round((agent.verifications.github?.verified ? 25 : 0) + (agent.verifications.solana?.verified ? 25 : 0) + (agent.verifications.x?.verified ? 25 : 0) + (agent.verifications.satp?.verified ? 25 : 0))) },
                { label: "Reputation", pct: Math.min(100, Math.round(displayRating * 20)) },
                { label: "Verification", pct: Math.min(100, agent.trustScore) },
                { label: "Activity", pct: Math.min(100, (agent.activity?.length || 0) * 10) },
                { label: "Job History", pct: Math.min(100, Math.max(agent.jobsCompleted, reviews.length) * 20) },
              ].map(({ label, pct }) => (
                <div key={label}>
            ...
            <div className="space-y-2 text-xs" style={{ fontFamily: "var(--font-mono)" }}>
              <div className="flex justify-between">
                ...
                {v.satp?.verified && v.solana?.address ? (
                  ...
                  >
                    {v.satp.did?.slice(0, 20)}... <ExternalLink size={10} />
                  </a>
                ) : (
      ...
      {/* Genesis Record */}
      {(agent as any).nftAvatar?.permanent && (
        <div className="mt-6">
          <h2 className="text-lg font-bold mb-3" style={{ fontFamily: "var(--font-mono)", color: "var(--text-primary)" }}>
            Genesis Record
          </h2>
          <BirthCertificate profileId={agent.id} profileName={agent.name} />
        </div>
      )}
    </div>
  );
